...
then
  echo "disjointClasses([b7,b8])." >> temp.pl
  echo "annotationAssertion('disponte:probability',disjointClasses([b7,b8]),literal('0.6'))." >> temp.pl
elif [[ $1 -eq 4 ]]
then
...
do
	...
	then
	  ...
	  echo "S is cputime,instanceOf(b8,x,Expl,[return_prob(Prob)]),E is cputime, Time is E - S." | swipl --stack_limit=8G temp.pl >> res_trill_inc.txt 2>> err_trill_inc.txt
	fi
