...
:- module(utility_translation_inc, [load_owl/1, load_owl_from_string/1, expand_all_ns/4, expand_all_ns/5, is_axiom/1]).
...
construct(M:A) :- trill_inc:axiom(M:A).
...
:- multifile trill_inc:axiom/1.

trill_inc:axiom(M:A) :- classAxiom(M:A).
trill_inc:axiom(M:A) :- propertyAxiom(M:A).
trill_inc:axiom(M:hasKey(A,B)) :- M:hasKey(A,B).
trill_inc:axiom(M:A) :- fact(M:A).
trill_inc:axiom(M:A) :- declarationAxiom(M:A).
...
retract_all_axioms(M) :-
        findall(M:A,trill_inc:axiom(M:A),Axioms),
...
load_owl_from_stream(S):-
  ...
  close(S),
  trill_inc:add_kb_prefixes(M:NSList),
  rdf_2_owl(M,'ont'),
  utility_translation_init(M),
...
:- multifile trill_inc:kb_prefixes/1.

trill_inc:kb_prefixes(M:L):-
  M:ns4query(L),!.

% Adds a list of kb prefixes into ns4query
:- multifile trill_inc:add_kb_prefixes/1.

trill_inc:add_kb_prefixes(_:[]):-!.

trill_inc:add_kb_prefixes(M:[(H=H1)|T]):-
  trill_inc:add_kb_prefix(M:H,H1),
  trill_inc:add_kb_prefixes(M:T).

% Adds a prefix into ns4query
:- multifile trill_inc:add_kb_prefix/2.

trill_inc:add_kb_prefix(M:'',B):- !,
  trill_inc:add_kb_prefix(M:[],B).

trill_inc:add_kb_prefix(M:A,B):-
  M:ns4query(L),!,
  (\+ member((A=_),L) ->
...
:- multifile trill_inc:remove_kb_prefix/2.
trill_inc:remove_kb_prefix(M:A,B):-
  M:ns4query(L),!,
  (member((A=B),L) ->
      (retract(M:ns4query(L)),
       delete(L,(A=B),NL),
...
:- multifile trill_inc:remove_kb_prefix/1.
trill_inc:remove_kb_prefix(M:A):-
  M:ns4query(L),!,
  (member((A=B),L) *->
      (retract(M:ns4query(L)),
       delete(L,(A=B),NL),
...
:- multifile trill_inc:add_axiom/1.
trill_inc:add_axiom(M:Ax):-
  assert(M:addKBName),
  init_kb_atom(M),
...
:- multifile trill_inc:add_axioms/1.
trill_inc:add_axioms(_:[]).

trill_inc:add_axioms(M:[H|T]) :-
  trill_inc:add_axiom(M:H),
  trill_inc:add_axioms(M:T).

:- multifile trill_inc:remove_axiom/1.
trill_inc:remove_axiom(M:Ax):-
  %print_message(warning,under_development),
  ( M:ns4query(NSList) -> true; NSList = []),
  expand_axiom(M,Ax,NSList,ExpAx),
  retract_axiom(M,ExpAx),
  retractall(M:owl(ExpAx,'ont')),!,
  trill_inc:reload_kb(M:false).


/*
trill_inc:remove_axiom(M:subClassOf(C,D)):-
  print_message(warning,under_development),
  ( M:ns4query(NSList) -> true; NSList = []),
...
:- multifile trill_inc:remove_axioms/1.
trill_inc:remove_axioms(_:[]):-!.

trill_inc:remove_axioms(M:[H|T]) :-
  trill_inc:remove_axiom(M:H),
  trill_inc:remove_axioms(M:T).
...
clean_up(M):-
  ...
  forall(trill_inc:axiom(M:A),retractall(M:A)),
...
sandbox:safe_primitive(utility_translation_inc:load_owl(_)).
sandbox:safe_primitive(utility_translation_inc:load_owl_from_string(_)).
sandbox:safe_primitive(utility_translation_inc:expand_all_ns(_,_,_,_)).
sandbox:safe_primitive(utility_translation_inc:expand_all_ns(_,_,_,_,_)).
%sandbox:safe_primitive(utility_translation_inc:query_expand(_)).
